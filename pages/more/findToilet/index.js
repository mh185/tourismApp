// pages/index/findToilet/index.js
import {
  findToilet
} from '../../../utils/more/tourGuide'
...
Page({
  ...
  toWC() {
    wx.openLocation({
      longitude: this.data.currentPoint.longitude,
      latitude: this.data.currentPoint.latitude,
      scale: 18,
      complete: res => {
        console.log(res)
      }
  ...
  gitWCList() {
    ...
    findToilet(params).then((res) => {
      // console.log('res7777777777', res)
      const l = {
        COMPANY_ENTERPRISE_MARKET: "单位、企业、商场公厕",
        ENVIRONMENTAL_SANITATION: "环卫公厕",
        GAS_STATION: "加油站公厕",
        MARKET: "市场公厕",
        GARDENS: "园林公厕",
      }
      if (res.code == 200) {
        ...
        that.getWc(that.data.mapStart, that.data.mapEnd)
  ...
  getWc(e, d) {
    const that = this
    // 调用接口
    if(d>that.data.wcList.length){
      d=that.data.wcList.length
    }
    qqmapsdk.search({
      keyword: '厕所', //搜索关键词
      ocation: {
      ...
      success: function (res) { //搜索成功后的回调
        ...
        var mks = []
        const wc = that.data.wcList
        for (var i = e; i < d; i++) {
          that.data.markers.push({ // 获取返回结果，放到mks数组中
            title: wc[i].title,
            id: Number(wc[i].distance),
            latitude: wc[i].lat,
            longitude: wc[i].lon,
